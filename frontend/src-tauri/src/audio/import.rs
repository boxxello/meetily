...
mod tests {
    ...
    fn test_import_pipeline_decode_vad() {
        ...
        let live_redemption_ms = 2000;
        println!(
            "\n--- Live-style VAD feed: 600ms windows, redemption_time={}ms, cap=20s ---",
            live_redemption_ms
        );
        let mut live_processor =
            crate::audio::vad::ContinuousVadProcessor::new(16000, live_redemption_ms)
                .expect("Failed to create live VAD processor")
                .with_max_live_segment_ms(20_000);
        let mut live_segments = Vec::new();
        for chunk in samples.chunks(9600) {
            live_segments.extend(
                live_processor
                    .process_audio(chunk)
                    .expect("live VAD failed"),
            );
        }
        live_segments.extend(live_processor.flush().expect("live VAD flush failed"));
        println!("Found {} live-style segments", live_segments.len());
        assert!(
            !live_segments.is_empty(),
            "Live-style VAD should detect speech after 16kHz preprocessing"
        );

        println!(
            "\n--- Live-style VAD feed: decoded native rate windows, redemption_time={}ms, cap=20s ---",
            live_redemption_ms
        );
        let native_mono = if decoded.channels > 1 {
            crate::audio::audio_processing::audio_to_mono(&decoded.samples, decoded.channels)
        } else {
            decoded.samples.clone()
        };
        let native_window = ((decoded.sample_rate as f64 * 0.600) as usize).max(1);
        let mut native_live_processor =
            crate::audio::vad::ContinuousVadProcessor::new(decoded.sample_rate, live_redemption_ms)
                .expect("Failed to create native live VAD processor")
                .with_max_live_segment_ms(20_000);
        let mut native_live_segments = Vec::new();
        for chunk in native_mono.chunks(native_window) {
            native_live_segments.extend(
                native_live_processor
                    .process_audio(chunk)
                    .expect("native live VAD failed"),
            );
        }
        native_live_segments.extend(
            native_live_processor
                .flush()
                .expect("native live VAD flush failed"),
        );
        println!(
            "Found {} native live-style segments",
            native_live_segments.len()
        );
        assert!(
            !native_live_segments.is_empty(),
            "Native-rate live-style VAD should detect speech"
        );
        assert!(
            native_live_segments.len() <= live_segments.len() + 1,
            "Native-rate live VAD fragmented much more than the 16kHz reference path: native={} reference={}",
            native_live_segments.len(),
            live_segments.len()
        );

        let reference_first_duration =
            live_segments[0].end_timestamp_ms - live_segments[0].start_timestamp_ms;
        let native_first_duration =
            native_live_segments[0].end_timestamp_ms - native_live_segments[0].start_timestamp_ms;
        assert!(
            native_first_duration >= reference_first_duration * 0.8,
            "Native-rate live VAD first segment was fragmented: native={:.0}ms reference={:.0}ms",
            native_first_duration,
            reference_first_duration
        );
    }
}
