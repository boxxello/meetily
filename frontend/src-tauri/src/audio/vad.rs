...
const PRE_SPEECH_PAD_MS: u64 = 300;
const POST_SPEECH_PAD_MS: u64 = 400;
const MIN_SPEECH_TIME_MS: u64 = 250;
...
impl ContinuousVadProcessor {
    pub fn new(input_sample_rate: u32, redemption_time_ms: u32) -> Result<Self> {
        ...
        // Keep the known-stable Silero thresholds here. More aggressive recall
        // settings have triggered silero-rs range panics on real recordings.
        ...
        config.pre_speech_pad = Duration::from_millis(PRE_SPEECH_PAD_MS);
        config.post_speech_pad = Duration::from_millis(POST_SPEECH_PAD_MS);
        config.min_speech_time = Duration::from_millis(MIN_SPEECH_TIME_MS);

        debug!("Creating VAD session with: sample_rate={}Hz, positive_threshold={:.2}, negative_threshold={:.2}, redemption={}ms, pre_pad={}ms, post_pad={}ms, min_speech={}ms, input_rate={}Hz",
               VAD_SAMPLE_RATE,
        ...
        Ok(Self {
            ...
            max_live_segment_samples: None,
            live_split_active: false,
            // Initialize state tracking
            last_logged_state: false,
        })
    }

    pub fn with_max_live_segment_ms(mut self, max_segment_ms: u32) -> Self {
        let max_samples = ((16000.0 * max_segment_ms as f32) / 1000.0) as usize;
        self.max_live_segment_samples = Some(max_samples.max(self.chunk_size));
        self
    }
    ...
    pub fn process_audio(&mut self, samples: &[f32]) -> Result<Vec<SpeechSegment>> {
        ...
        while self.buffer.len() >= self.chunk_size {
            ...
            self.emit_live_segment_if_needed();
    ...
    /// Uses the same sinc-based resampler as file imports. The previous
    /// linear interpolator fragmented live speech after 48kHz -> 16kHz
    /// conversion, while the import path on the same saved audio did not.
    fn resample_to_16k(&self, samples: &[f32]) -> Result<Vec<f32>> {
        if self.sample_rate == 16000 {
            return Ok(samples.to_vec());
        }

        let mut resampled = super::audio_processing::resample(samples, self.sample_rate, 16000)
            .map_err(|e| anyhow!("VAD resampling failed: {}", e))?;
        ...
        debug!(
            "Resampled from {} samples ({}Hz) to {} samples (16kHz) for VAD",
            samples.len(),
            self.sample_rate,
            resampled.len()
        );
        ...
        for transition in transitions {
            match transition {
                ...
                VadTransition::SpeechEnd {
                    ...
                    // After live splitting, emit only the not-yet-transcribed tail.
                    let speech_samples = if self.live_split_active {
                    ...
                    if !speech_samples.is_empty() {
                        let (segment_start_ms, segment_end_ms) = if self.live_split_active {
                            let start_ms = (self.speech_start_sample as f64 / 16000.0) * 1000.0;
                            let end_sample = self.speech_start_sample + speech_samples.len();
    ...
    fn emit_live_segment_if_needed(&mut self) {
        let Some(max_samples) = self.max_live_segment_samples else {
            return;
        };

        while self.in_speech && self.current_speech.len() >= max_samples {
            let segment_samples: Vec<f32> = self.current_speech.drain(..max_samples).collect();
            let start_sample = self.speech_start_sample;
            let end_sample = start_sample + segment_samples.len();
            let start_ms = (start_sample as f64 / 16000.0) * 1000.0;
            let end_ms = (end_sample as f64 / 16000.0) * 1000.0;

            info!(
                "VAD: Emitting capped live segment: {:.1}ms duration, {} samples",
                end_ms - start_ms,
                segment_samples.len()
            );
...
mod tests {
    ...
    #[test]
    fn test_live_segment_cap_emits_during_continuous_speech() {
        let mut processor = ContinuousVadProcessor::new(16000, 400)
            .expect("Failed to create processor")
            .with_max_live_segment_ms(5000);

        processor.in_speech = true;
        processor.speech_start_sample = 0;
        processor.current_speech = vec![0.1; 16000 * 12];

        processor.emit_live_segment_if_needed();

        assert_eq!(processor.speech_segments.len(), 2);
        assert_eq!(processor.current_speech.len(), 16000 * 2);
        assert_eq!(processor.speech_start_sample, 16000 * 10);
        assert!(processor.live_split_active);

        let first = processor
            .speech_segments
            .pop_front()
            .expect("first segment");
        assert_eq!(first.samples.len(), 16000 * 5);
        assert_eq!(first.start_timestamp_ms, 0.0);
        assert_eq!(first.end_timestamp_ms, 5000.0);

        let second = processor
            .speech_segments
            .pop_front()
            .expect("second segment");
        assert_eq!(second.samples.len(), 16000 * 5);
        assert_eq!(second.start_timestamp_ms, 5000.0);
        assert_eq!(second.end_timestamp_ms, 10000.0);
    }
}
