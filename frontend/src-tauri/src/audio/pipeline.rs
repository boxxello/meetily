...
const LIVE_TRANSCRIPTION_MAX_SEGMENT_MS: u32 = 20_000;
const LIVE_TRANSCRIPTION_DEFAULT_CHUNK_MS: u32 = 8_000;
const LIVE_TRANSCRIPTION_MIN_CHUNK_MS: u32 = 2_000;
const LIVE_TRANSCRIPTION_MIN_ENERGY: f32 = 1.0e-6;
const MIXING_WINDOW_MS: f32 = 50.0;
const MIXING_MAX_BUFFER_MULTIPLIER: usize = 8;

#[cfg(test)]
fn live_transcription_max_segment_ms() -> u32 {
    LIVE_TRANSCRIPTION_MAX_SEGMENT_MS
}

fn live_transcription_chunk_ms(target_chunk_duration_ms: u32) -> u32 {
    let requested = if target_chunk_duration_ms == 0 {
        LIVE_TRANSCRIPTION_DEFAULT_CHUNK_MS
    } else {
        target_chunk_duration_ms
    };

    requested.clamp(
        LIVE_TRANSCRIPTION_MIN_CHUNK_MS,
        LIVE_TRANSCRIPTION_MAX_SEGMENT_MS,
    )
}
...
struct LiveTranscriptionBuffer {
    sample_rate: u32,
...
impl LiveTranscriptionBuffer {
    fn new(sample_rate: u32, chunk_duration_ms: u32) -> Self {
        let chunk_size_samples = ((sample_rate as f64 * chunk_duration_ms as f64) / 1000.0)
            .round()
            .max(1.0) as usize;

        info!(
            "Live transcription buffer initialized: chunk={}ms ({} samples), min_energy={:.8}",
            chunk_duration_ms, chunk_size_samples, LIVE_TRANSCRIPTION_MIN_ENERGY
        );

        Self {
            sample_rate,
            chunk_size_samples,
            buffered_samples: Vec::with_capacity(chunk_size_samples * 2),
            next_start_sample: 0,
        }
    }

    fn push_samples(&mut self, samples: &[f32]) -> Vec<LiveTranscriptionChunk> {
        self.buffered_samples.extend_from_slice(samples);

        let mut chunks = Vec::new();
        while self.buffered_samples.len() >= self.chunk_size_samples {
            let chunk_samples = self
                .buffered_samples
                .drain(..self.chunk_size_samples)
                .collect::<Vec<_>>();
            if let Some(chunk) = self.build_chunk(chunk_samples) {
                chunks.push(chunk);
            }
        }
    ...
    fn flush(&mut self) -> Option<LiveTranscriptionChunk> {
        if self.buffered_samples.is_empty() {
            return None;
        }

        let chunk_samples = self.buffered_samples.drain(..).collect::<Vec<_>>();
        self.build_chunk(chunk_samples)
    }

    fn build_chunk(&mut self, samples: Vec<f32>) -> Option<LiveTranscriptionChunk> {
        let timestamp_sec = self.next_start_sample as f64 / self.sample_rate as f64;
        self.next_start_sample += samples.len() as u64;

        if is_speech_like_live_audio(&samples) {
            Some(LiveTranscriptionChunk {
                samples,
                timestamp_sec,
            })
        } else {
            debug!(
                "Skipping low-energy live transcription chunk at {:.2}s ({} samples)",
                timestamp_sec,
                samples.len()
            );
            None
...
fn is_speech_like_live_audio(samples: &[f32]) -> bool {
    if samples.is_empty() {
        return false;
    }

    let energy = samples.iter().map(|sample| sample * sample).sum::<f32>() / samples.len() as f32;
    energy >= LIVE_TRANSCRIPTION_MIN_ENERGY
}
...
impl AudioMixerRingBuffer {
    fn new(sample_rate: u32) -> Self {
        ...
        let window_ms = MIXING_WINDOW_MS;
        ...
        let max_buffer_size = window_size_samples * MIXING_MAX_BUFFER_MULTIPLIER; // 400ms

        info!(
            "🔊 Ring buffer initialized: window={}ms ({} samples), max={}ms ({} samples)",
            window_ms,
            window_size_samples,
            window_ms * MIXING_MAX_BUFFER_MULTIPLIER as f32,
            max_buffer_size
        );
...
/// Mixes microphone/system audio, records the mixed stream, and sends fixed
/// live chunks to ASR. VAD is intentionally not a hard live gate: on real
/// system-audio recordings Silero can miss most speech-like audio, which loses
/// words before ASR has a chance to decode them.
pub struct AudioPipeline {
    ...
    transcription_buffer: LiveTranscriptionBuffer,
...
impl AudioPipeline {
    pub fn new(
        ...
        let chunk_duration_ms = live_transcription_chunk_ms(target_chunk_duration_ms);
        let transcription_buffer = LiveTranscriptionBuffer::new(sample_rate, chunk_duration_ms);
    ...
    /// Run the live audio processing pipeline.
    pub async fn run(mut self) -> Result<()> {
        info!(
            "Live audio pipeline started - fixed audio chunks are sent to ASR with energy-only silence skipping"
        );
                ...
                Ok(Some(chunk)) => {
                    ...
                    while self.ring_buffer.can_mix() {
                        if let Some((mic_window, sys_window)) = self.ring_buffer.extract_window() {
                            ...
                            // STEP 3: Send mixed audio for transcription. Use fixed chunks
                            // rather than hard VAD gating so speech-like system audio cannot
                            // be erased before ASR sees it.
                            for live_chunk in
                                self.transcription_buffer.push_samples(&mixed_with_gain)
                            {
                                self.send_live_transcription_chunk(live_chunk);
                            }
    ...
    fn flush_remaining_audio(&mut self) -> Result<()> {
        ...
        if let Some(final_chunk) = self.transcription_buffer.flush() {
            self.send_live_transcription_chunk(final_chunk);
        }

        Ok(())
    }

    fn send_live_transcription_chunk(&mut self, live_chunk: LiveTranscriptionChunk) {
        let duration_ms = live_chunk.samples.len() as f64 / self.sample_rate as f64 * 1000.0;
        info!(
            "📤 Sending live transcription chunk: {:.1}ms, {} samples",
            duration_ms,
            live_chunk.samples.len()
        );

        let transcription_chunk = AudioChunk {
            data: live_chunk.samples,
            sample_rate: self.sample_rate,
            timestamp: live_chunk.timestamp_sec,
            chunk_id: self.chunk_id_counter,
            device_type: DeviceType::Microphone,
        };

        if let Err(e) = self.transcription_sender.send(transcription_chunk) {
            warn!("Failed to send live transcription chunk: {}", e);
        } else {
...
mod tests {
    ...
    #[test]
    fn live_transcription_forced_split_keeps_whisper_context() {
        let max_segment_ms = live_transcription_max_segment_ms();

        assert!(
            max_segment_ms >= 15_000,
            "forced live transcription splits need enough context for Whisper"
        );
        assert!(
            max_segment_ms <= 25_000,
            "forced live transcription splits should stay below Whisper's 30s window"
        );
    }

    #[test]
    fn live_transcription_defaults_to_responsive_fixed_chunks() {
        let chunk_ms = live_transcription_chunk_ms(0);

        assert_eq!(chunk_ms, LIVE_TRANSCRIPTION_DEFAULT_CHUNK_MS);
        assert!(chunk_ms >= LIVE_TRANSCRIPTION_MIN_CHUNK_MS);
        assert!(chunk_ms <= live_transcription_max_segment_ms());
    }

    #[test]
    fn live_transcription_buffer_covers_vad_missed_energized_audio() {
        let sample_rate = 48_000;
        let mut buffer = LiveTranscriptionBuffer::new(sample_rate, 8_000);
        let samples = vec![0.02; sample_rate as usize * 18];

        let mut chunks = buffer.push_samples(&samples);
        chunks.extend(buffer.flush());

        assert_eq!(chunks.len(), 3);
        assert_eq!(chunks[0].timestamp_sec, 0.0);
        assert!((chunks[1].timestamp_sec - 8.0).abs() < f64::EPSILON);
        assert!((chunks[2].timestamp_sec - 16.0).abs() < f64::EPSILON);

        let covered_sec = chunks
            .iter()
            .map(|chunk| chunk.samples.len() as f64 / sample_rate as f64)
            .sum::<f64>();
        assert!((covered_sec - 18.0).abs() < 0.001);
    }

    #[test]
    fn live_transcription_buffer_skips_silence_without_collapsing_timeline() {
        let sample_rate = 48_000;
        let mut buffer = LiveTranscriptionBuffer::new(sample_rate, 8_000);

        let silence = vec![0.0; sample_rate as usize * 8];
        let speech_like = vec![0.02; sample_rate as usize * 8];

        assert!(buffer.push_samples(&silence).is_empty());
        let chunks = buffer.push_samples(&speech_like);

        assert_eq!(chunks.len(), 1);
        assert!((chunks[0].timestamp_sec - 8.0).abs() < f64::EPSILON);
    }

    #[test]
    fn ring_buffer_uses_low_latency_mixing_window() {
        let ring_buffer = AudioMixerRingBuffer::new(48_000);

        assert_eq!(ring_buffer.window_size_samples, 2_400);
        assert_eq!(ring_buffer.max_buffer_size, 19_200);
    }

    #[test]
    fn ring_buffer_mixes_microphone_without_system_audio() {
        let mut ring_buffer = AudioMixerRingBuffer::new(48_000);
        let mic_samples = vec![0.1; ring_buffer.window_size_samples];

        ring_buffer.add_samples(DeviceType::Microphone, mic_samples);

        assert!(ring_buffer.can_mix());
        let (mic_window, sys_window) = ring_buffer.extract_window().expect("mixed window");
        assert!(mic_window.iter().any(|sample| *sample != 0.0));
        assert!(sys_window.iter().all(|sample| *sample == 0.0));
    }

    #[test]
    fn ring_buffer_mixes_system_audio_without_microphone() {
        let mut ring_buffer = AudioMixerRingBuffer::new(48_000);
        let sys_samples = vec![0.1; ring_buffer.window_size_samples];

        ring_buffer.add_samples(DeviceType::System, sys_samples);

        assert!(ring_buffer.can_mix());
        let (mic_window, sys_window) = ring_buffer.extract_window().expect("mixed window");
        assert!(mic_window.iter().all(|sample| *sample == 0.0));
        assert!(sys_window.iter().any(|sample| *sample != 0.0));
    }
}
